...
async def get_reception_depots(
    current_user: User = Depends(get_current_user),
    movement_service: MovementService = Depends(get_movement_service),
):
    """Get list of available depots for reception filtering."""
    if current_user.role not in [Role.ADMIN, Role.MARKETER, Role.STOCK_GESTIONNAIRE]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
...
async def get_reception_products(
    current_user: User = Depends(get_current_user),
    movement_service: MovementService = Depends(get_movement_service),
):
    """Get list of available products for reception filtering."""
    if current_user.role not in [Role.ADMIN, Role.MARKETER, Role.STOCK_GESTIONNAIRE]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
...
async def get_reception_distributors(
    current_user: User = Depends(get_current_user),
    movement_service: MovementService = Depends(get_movement_service),
):
    """Get list of available distributors for reception filtering."""
    if current_user.role not in [Role.ADMIN, Role.MARKETER, Role.STOCK_GESTIONNAIRE]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
...
@router.get("/metadata/origins")
async def get_reception_origins(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Get list of available origins for reception filtering from TOrigine."""
    if current_user.role not in [Role.ADMIN, Role.MARKETER, Role.STOCK_GESTIONNAIRE]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
    
    from app.models.scdp import TOrigine
    from sqlalchemy import select
    
    query = (
        select(TOrigine.code_orig, TOrigine.orig_nom)
        .order_by(TOrigine.orig_nom)
    )
    ...
    return [
        {
            "code": item.code_orig,
            "name": item.orig_nom or item.code_orig,
        }
        for item in items
    ]
...
async def preview_receptions_export(
    depot_code: Optional[str] = Query(None),
    product_code: Optional[str] = Query(None),
    distributor_code: Optional[str] = Query(None),
    search: Optional[str] = Query(None),
    ...
    """Preview export data for receptions with filters applied."""
    ...
    return {
        "document": {
            "title": "Bordereau de Réception de Stock",
...
async def export_receptions_csv(
    depot_code: Optional[str] = Query(None),
    product_code: Optional[str] = Query(None),
    distributor_code: Optional[str] = Query(None),
    search: Optional[str] = Query(None),
    ...
    for item in items:
        writer.writerow([
            item.get("receptionDate", "") if item.get("receptionDate") else "",
            item.get("receptionNumber", "") or f"REC-{item.get('id', '')}",
    ...
    return StreamingResponse(
        ...
        media_type='text/csv',
        headers={
            'Content-Disposition': f'attachment; filename="receptions_{current_user.email}.csv"'
        }
    )
