...
export default function ActivateAdmin() {
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || "Code de verification incorrect ou expire.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      await otpApi.sendOtp(email, "ACCOUNT_VERIFICATION");
      setError("Un nouveau code a ete envoye.");
        ...
        <div className="bg-card rounded-2xl border border-border shadow-soft p-8">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-heading font-bold tracking-tight text-primary">
              {success ? "Compte active" : "Activer votre compte Administrateur"}
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              {success
                ? "Votre compte administrateur a ete active avec succes."
                : "Suivez les etapes pour configurer votre mot de passe et activer votre compte."}
            </p>
          </div>
          ...
          {success ? (
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="admin@scdp.cm" value={email}
                  onChange={(e) => setEmail(e.target.value)} required disabled={loading}
                />
                <p className="mt-2 text-xs text-muted-foreground">
                  Entrez l'adresse e-mail associee a votre compte administrateur.
                </p>
              </div>
              <button type="submit"
            ...
            <form onSubmit={handleOtpSubmit} className="space-y-5">
              ...
              <div>
                <label className="form-label" htmlFor="otp">
                  Code de verification (OTP) <span className="text-destructive">*</span>
                </label>
                <input
                  id="otp" name="otp" type="text" className="form-input"
                  placeholder="123456" value={otp}
                  onChange={(e) => setOtp(e.target.value)} required maxLength={6} disabled={loading}
                />
                <p className="mt-2 text-xs text-muted-foreground">
                  Entrez le code a 6 chiffres envoye a votre e-mail.
                </p>
              </div>
              ...
              <button type="button" onClick={() => setStep(1)}
                className="w-full text-sm text-muted-foreground hover:text-primary transition" disabled={loading}>
                Retour
              </button>
            </form>
          ) : (
            <form onSubmit={handlePasswordSubmit} className="space-y-5">
              ...
              <div>
                ...
                />
                <p className="mt-2 text-xs text-muted-foreground">Minimum 8 caracteres.</p>
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setConfirmPassword(e.target.value)} required minLength={8} disabled={loading}
                />
              </div>
              <button type="submit"
                className="w-full min-h-11 rounded-xl bg-primary px-5 text-sm font-semibold text-primary-foreground shadow-soft hover:brightness-105 transition"
